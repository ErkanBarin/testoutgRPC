...
// Client factory function - directly create the client using generated stubs with SSL
function createSecureLimitsClient(address) {
  console.log(`Creating secure Limits client for ${address}`);
  ...
  try {
    ...
    // Create SSL credentials with our public key
    const sslCreds = grpc.credentials.createSsl(
      null, // rootCerts (null means use default)
      null, // privateKey (null since we're only using public key)
      null  // certChain (null since we're only using public key)
    );
    
    // Add call credentials (metadata) to the SSL credentials
    ...
    console.error('Error creating secure limits client:', error.message);
    throw error;
  }
}

test.describe('Secure Limits API Test with SSL Credentials', () => {
  ...
  test.beforeAll(async () => {
    try {
      ...
      limitsClient = createSecureLimitsClient(LAB_GATEWAY_ADDRESS);
      
      console.log(`Using existing user: ${EXISTING_USER.email} with ID: ${userId}`);
      console.log(`Token validity: ${userToken ? 'Valid' : 'Invalid'}`);
    } catch (error) {
      console.error(`Failed to set up secure client test: ${error.message}`);
  ...
  test('Verify secure connection to t1 environment using SSL', async ({ page }) => {
    console.log("\n=== Verify secure connection to t1 environment using SSL ===");
